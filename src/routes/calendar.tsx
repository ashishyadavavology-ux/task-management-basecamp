...
function CalendarPage() {
  ...
  return (
    <AppShell title="Calendar" action={<PageHeaderAction label="New event" />}>
            ...
            {days.map((day) => {
              ...
              return (
                <div key={day.toISOString()} className={cn(
                  ...
                  isSameDay(day, new Date(2026, 5, 9)) && "ring-2 ring-primary",
                )}>
                  <span className="text-xs text-muted-foreground">{format(day, "d")}</span>
                  <div className="mt-1 space-y-1">
                    {dayEvents.map((e) => (
                      <div key={e.id} className={cn("truncate rounded px-1.5 py-0.5 text-[10px] font-medium", typeColor[e.type])}>{e.title}</div>
                    ))}
                  </div>
                </div>
              );
